"""Evaluate a trained agent on every historical walk-forward week.
...
def run(market, predict_fn, name, model_path=None, check_weeks=6, save_actions=False, results_dir=None):
    ...
    print(f"[{name}] {market}: deterministic and no lookahead on {check_weeks} sample weeks "
          f"(perturbation changed later actions in {changed:.0%} of cases)", flush=True)
...
def main():
    ...
    ap.add_argument("--model", required=True, help="path to a MaskablePPO .zip")
    ap.add_argument("--name", required=True, help="method label, e.g. rl_ou_s0")
    ...
    a = ap.parse_args()
    if a.name in {"dp", "pf"}:
        ap.error("--name must not overwrite the dp or pf rows")

    from sb3_contrib import MaskablePPO          # imported here so run() works without SB3
    model = MaskablePPO.load(a.model, device="cpu")
    run(a.market, sb3_predict_fn(model), a.name, model_path=a.model, check_weeks=a.check_weeks,
