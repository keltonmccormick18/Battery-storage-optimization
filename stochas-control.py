...
def seasonal_price(t, start_date, seasonal_model):
    #predicts seasonal price based on our basic seasonal regression.
    dt_now = start_date + timedelta(hours = t)
    row = pd.DataFrame({
        "hour_of_day": [dt_now.hour],
        "dow": [dt_now.weekday()],
        "month": [dt_now.month],
    })
    dummies = pd.get_dummies(row, columns = ["hour_of_day", "dow", "month"], drop_first = True, dtype = float)
    dummies = sm.add_constant(dummies)
    dummies = dummies.reindex(columns = seasonal_model.params.index, fill_value = 0)
    return seasonal_model.predict(dummies).values[0]
